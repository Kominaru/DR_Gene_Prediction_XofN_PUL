...
def construct_xofn_features(x, y, seed):
    """
    ...
    clf = DecisionTreeClassifier(min_samples_leaf=MIN_SAMPLES_LEAF, class_weight='balanced', random_state=seed)
    clf.fit(x, y)
...
def compute_xofn(x, path_final):
    ...
    x_of_n = pd.DataFrame(x_of_n, columns=[f'X_of_N_{i+1}' for i in range(x_of_n.shape[1])])
